...
def main():
    ...
    parser.add_argument('--discrimination_path', type=str, help='path of the discrimination score')
    ...
    disc_dict = {}
    with open(args.discrimination_path, 'r') as file:
        for line in file:
            line = line.strip()
            if ':' in line:
                key, value = line.split(':', 1)
                disc_dict[key.strip()] = value.strip()

    trainer.train_model(args, model, device, dataloaders, criterion, optimizer_ft, num_epochs=args.epochs, disc_dict=disc_dict,
                check_point=1)
