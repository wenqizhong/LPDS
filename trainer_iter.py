...
def train_model(args, model, device, dataloaders, criterion, optimizer, num_epochs, disc_dict=None, times=None, check_point=1):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            cnt = 0
            for iteration, (img_name, img, scanpaths, labels, trans_factor) in enumerate(dataloaders[phase]):
                ...
                with torch.autograd.set_grad_enabled(phase == 'train'):
                    ...
                    if disc_dict is not None:
                        stimuli_weights = disc_dict[img_name]
                        loss_per_stimuli = criterion(outputs, labels)
                        weighted_loss = loss_per_stimuli * stimuli_weights
